...
import { useBopomofo, type BopomofoChar } from "@/hooks/useBopomofo";
...
export function TermMode({ card, status, onSubmit }: TermModeProps) {
  ...
  useEffect(() => {
    ...
    // 檢查是否填滿 (無 undefined)
    const isFilled = userInputs.length === targetLength && !userInputs.includes(undefined);

    if (status === 'question' && isFilled) {
      checkAnswer(userInputs as BopomofoChar[]);
    }
  }, [userInputs, status, card]);
      ...
      <div className="flex flex-wrap justify-center gap-4">
        {blocks.map((block, index) => {
          ...
          if (status === 'success') {
             ...
             const iStr = (inputChar?.initial || "") + (inputChar?.medial || "") + (inputChar?.final || "") + iTone;
          ...
          return (
            <div key={`${card.id}-${index}`} onClick={(e) => { e.stopPropagation(); handleBlockClick(index); }}>
              <CharacterBlock
                char={block.char} 
