...
export function ImportDeckDialog({ onSuccess }: ImportDeckDialogProps) {
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // 1. 建立題庫
      const deckRef = await DeckService.createDeck(user.uid, data.title, data.description);
      const newDeckId = deckRef.id;

      // 如果有 tags，更新上去 (假設 createDeck 還沒支援 tags 參數)
      if (Array.isArray(data.tags)) {
        await DeckService.updateDeck(newDeckId, { tags: data.tags, isPublic: !!data.isPublic });
      }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription>請貼上包含 title 與 cards 的 JSON 物件。</DialogDescription>
        </DialogHeader>
        ...
        <div className="flex justify-end gap-2 shrink-0">
          <Button variant="outline" onClick={() => setJsonInput(exampleJson)}>載入範例</Button>
