...
export default function AdCenter() {
  ...
  const handleDonate = async () => {
    ...
    if(confirm("確認支付 $1 USD? (模擬)")) {
        await PointsService.updatePoints(user.uid, 1000, "donation", "捐贈 $1 USD");
      ...
      <Tabs defaultValue="earn" className="space-y-4">
        ...
        <TabsContent value="earn" className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {ads.map(ad => (
                    <Card key={ad.id} className="hover:border-primary transition-colors cursor-pointer" onClick={() => handleWatch(ad)}>
                        <CardHeader>
                            <CardTitle className="flex justify-between items-start">
                                {ad.title}
                                <Badge className="bg-yellow-500 hover:bg-yellow-600">+{ad.rewardPoints} 分</Badge>
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <div className="flex gap-2 flex-wrap">
                                {ad.targetTags.map(t => <Badge key={t} variant="outline" className="text-xs">#{t}</Badge>)}
                            </div>
                        </CardContent>
                ...
                {ads.length === 0 && <div className="p-8 text-center text-muted-foreground">目前沒有適合您的廣告。</div>}
            </div>
        </TabsContent>
        ...
        <TabsContent value="donate">
            <Card>
                <CardHeader><CardTitle>支持 Nous Slate 開發</CardTitle></CardHeader>
                <CardContent>
                    <p className="mb-4">您的支持是我們持續維護伺服器的動力。每捐贈 1 美金，將回饋您 1000 積分。</p>
                    <Button onClick={handleDonate} className="w-full sm:w-auto gap-2 bg-pink-600 hover:bg-pink-700">
        ...
        <TabsContent value="advertise">
            <Card>
                <CardHeader><CardTitle>刊登您的廣告</CardTitle></CardHeader>
                <CardContent className="space-y-4">
                    <div className="grid gap-2">
                        <Label>廣告標題</Label>
                        <Input value={adTitle} onChange={e => setAdTitle(e.target.value)} placeholder="例如：最強國文補習班" />
                    </div>
                    <div className="grid gap-2">
                        <Label>目標受眾標籤 (逗號分隔)</Label>
                        <Input value={adTags} onChange={e => setAdTags(e.target.value)} placeholder="國文, 考試, 升學" />
                    </div>
                    <div className="grid gap-2">
                        <Label>出價 (每次觀看支付積分)</Label>
                        <Input type="number" value={adBid} onChange={e => setAdBid(Number(e.target.value))} />
                        <p className="text-xs text-muted-foreground">出價越高，在推薦列表中的排名越靠前。</p>
                    </div>
