import { BookOpen, Globe, Lock as LockIcon, MoreVertical, Pen, Plus, Trash2 } from "lucide-react";
    ...
    <div className="container mx-auto p-8 space-y-8">
      ...
      {decks.length === 0 ? (
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {decks.map((deck) => (
            <Card key={deck.id} className="group hover:shadow-md transition-shadow flex flex-col">
              ...
              <CardFooter className="pt-4 border-t bg-slate-50/50 dark:bg-slate-900/50 flex justify-between items-center text-sm">
                ...
                <Link to={`/editor/${deck.id}`}>
                  <Button variant="secondary" size="sm">進入編輯</Button>
                </Link>
              </CardFooter>
            </Card>
          ))}
