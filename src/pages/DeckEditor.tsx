...
import { useMoedict, type MoedictHeteronym } from "@/hooks/useMoedict";
...
// Components
import { CardForm, type DeckEditorFormData } from "@/components/editor/deck-editor/CardForm";
...
export default function DeckEditor() {
  ...
  const [polyphoneCandidates, setPolyphoneCandidates] = useState<MoedictHeteronym[]>([]);
  ...
  const form = useForm<DeckEditorFormData>({
    defaultValues: {
      type: "term", stem: "", zhuyinRaw: "", definition: "", audioUrl: "",
  ...
  const handleSubmit = async (data: DeckEditorFormData) => {
    ...
    try {
      ...
      form.reset({
        type: data.type, stem: "", zhuyinRaw: "", definition: "", audioUrl: "",
        answer: "", option1: "", option2: "", option3: ""
      });
      fetchCards();
    } catch {
  ...
  const handleSelectCard = (card: CardType) => {
    ...
    let formData: Partial<DeckEditorFormData> = {
      type: card.type,
      stem: content.stem,
