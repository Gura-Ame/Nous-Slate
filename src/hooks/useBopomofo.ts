...
export function useBopomofo(
  ...
  const processInput = useCallback((charOrTone: string) => {
  ...
  const processBackspace = useCallback(() => {
    ...
    // 檢查是否為空
    const isEmpty = !current.initial && !current.medial && !current.final && !current.tone;
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    ...
    if ([" ", "3", "4", "6", "7"].includes(key)) {
      e.preventDefault();
      const toneMap: Record<string, string> = { " ": " ", "6": "ˊ", "3": "ˇ", "4": "ˋ", "7": "˙" };
